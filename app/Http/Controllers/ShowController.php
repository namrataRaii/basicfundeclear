...
class ShowController extends Controller
{
    ...
    {
        $tableName = (new ShowModel)->getTable();
        $data['tablename'] = $tableName;
        if ($id != '') {

            $id = decrypt($id);
            $data['editshow'] = ShowModel::where('id', $id)->first();
        } else {
            $data['editshow'] = '';
        }
       
        $data['title'] = 'Shows';
        $data['shows'] = ShowModel::where('status', 1)->get();
        if ($request->ajax()) {
            $data = ShowModel::orderBy('position_by', 'asc')
            -> get();
                ...
                ->addColumn('action', function ($row) use ($tableName) {
                    ...
                    $actionBtn = '<div class="dropdown d-inline-block user-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            <a class="dropdown-item" href="' . route('admin.editshow', ['id' => $encryptedId]) . '">
        ...
        return view('admin.manage-show', $data);
    }
    ...
    public function save(Request $request)
{
    ...
    $total = ShowModel::count();
    $position_by = $total + 1;
    // Handle file upload
    if ($request->hasFile('thumbnail')) {
        $imagePath = $request->file('thumbnail')->store('show', 's3');

        // Delete the old thumbnail if updating an existing record
        if (!empty($request->id)) {
            $show = ShowModel::find($request->id);
            if (!empty($show->thumbnail)) {
                $fileName = basename($show->thumbnail);

                // Delete the file from S3
                Storage::disk('s3')->delete('show/' . $fileName);
            }
        }
    }
