...
class ShowAssignController extends Controller
{
    public function index(Request $request, $id = '')
    {
        ...
        $tableName = (new ShowAssignModel)->getTable();
        $data['tablename'] = $tableName;
        $data['title'] = 'Show Assign';
        $data['showtypes'] = ShowTypeModel::where('status', 1)->get();
        if ($id != '') {

            $id = decrypt($id);
            $data['editshow'] = ShowAssignModel::where('id', $id)->first();
        } else {
            $data['editshow'] = '';
        }

        $data['title'] = 'Shows';
        $data['shows'] = ShowAssignModel::where('status', 1)->get();
        if ($request->ajax()) {
            $data = ShowAssignModel::orderBy('position_by', 'asc')
                ->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) use ($tableName) {
                    ...
                    $actionBtn = '<div class="dropdown d-inline-block user-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            <a class="dropdown-item" href="' . route('admin.editshowassign', ['id' => $encryptedId]) . '">
                ...
                ->addColumn('show_type', function ($row) {
                    return $row->showType ? $row->showType->title : 'N/A';
                })
    ...
    {
        $total = ShowAssignModel::count();
        $position_by = $total + 1;
        // Handle file upload
        if ($request->hasFile('thumbnail')) {
            $imagePath = $request->file('thumbnail')->store('showassign', 's3');

            // Delete the old thumbnail if updating an existing record
            if (!empty($request->id)) {
                $show = ShowAssignModel::find($request->id);
                $fileName = basename($show->thumbnail);

                // Delete the file from S3
                Storage::disk('s3')->delete('showassign/' . $fileName);
            }
        }
        ...
        if (!empty($request->id)) {
            ...
            $request->validate([
                'showtype_id' => 'required|string|max:255',
            ...
            if (!empty($show)) {
                $show->update([
                    'showtype_id' => $request->showtype_id,
                    'title' => $request->title,
                ...
                Session::flash('error', 'Show with ID ' . $request->id . ' not found.');
            }
        } else {
            $request->validate([
                'showtype_id' => 'required|max:255',
            ...
            $show = new ShowAssignModel();
            $show->showtype_id = $request->showtype_id;
            ...
            $show->save();
            Session::flash('success', 'Show added successfully!');
        }

        // Redirect back with success or error message
        return redirect()->route('admin.showassign');
    }
