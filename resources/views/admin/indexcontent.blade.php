                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="post" id="addForm" action="{{ route('admin.addindexcontent') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    ...
                                    <div class="col-sm-12 mt-1">
                                        <div class="row">
                                            ...
                                            <div class="col-sm-6">
                                                <div class="form-group">
                                                    <label class="form-label">Attribute <span class="text-danger fw-bold">*</span></label>
                                                    <input type="text" class="form-control attribute" name="attribute" value="{{ old('attribute', !empty($editcontent) ? $editcontent->attribute : '') }}" required>
                                                    @error('attribute')
                                                    <span class="text-danger">{{ $message }}</span>
                                                    @enderror
                                                </div>
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="form-group">
                ...
                <input type="file" id="fileInput" class="form-control" name="content" accept="image/*,video/*" {{ empty($editcontent) ? 'required' : '' }}>
                @error('content')
        ...
        <div class="col-sm-6">
            <div class="form-group">
                <label>Watch Link <span class="text-danger fw-bold">*</span></label>
                <div class="input-group">
                    <input type="url" class="form-control" name="watchlink" value="{{ old('watchlink', !empty($editcontent) ? $editcontent->watchlink : '') }}" required>
                    @if(!empty($editcontent))
                    <span class="input-group-text" id="showVideoBtn"> <a href="{{ $editcontent->watchlink}}" target="_blank"><i class="mdi mdi-eye"></i></a></span>
                    @endif
                    @error('watchlink')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
...
<div id="filePreview" class="d-none" style="width: 300px; display: flex; justify-content: center; align-items: center; overflow: hidden;">
    ...
    @endsection
    @section('externaljs')
    <script src="{{asset('assets/js/indexcontent.js')}}"></script>
    <script>
        ...
        //                 $('<img>').attr('src', URL.createObjectURL(file)).css({ maxWidth: '100%', maxHeight: '100%' }).appendTo(preview);
        //                 preview.removeClass('d-none');
        //             } else if (fileType.startsWith('video/')) {
        //                 $('<video>').prop({ controls: true, src: URL.createObjectURL(file), autoplay: true }).css({ maxWidth: '100%', maxHeight: '100%' }).appendTo(preview);
        //                 preview.removeClass('d-none');
        //             } else {
